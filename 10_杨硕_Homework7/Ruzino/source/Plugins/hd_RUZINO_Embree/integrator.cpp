...
Color Integrator::IntersectLights(const GfRay& ray, GfVec3f& intersectPos)
{
    float depth;
    Color radiance;
    auto* light = IntersectClosestLight(ray, intersectPos, radiance, depth);
    TF_UNUSED(light);
    return radiance;
}

Hd_RUZINO_Light* Integrator::IntersectClosestLight(
    const GfRay& ray,
    ...
    float& outDepth)
{
    ...
    Hd_RUZINO_Light* closestLight = nullptr;
    ...
    const std::function<float()>& uniform_float)
{
    ...
    auto brdf_pdf = si.Pdf(wi, si.wo);
    GfVec3f contribution_by_sample_lights{ 0 };

    if (this->VisibilityTest(
            si.position + 0.0001f * si.geometricNormal, sampled_light_pos) &&
        sample_light_pdf > 1e-6f) {
        float mis_weight = PowerHeuristic(sample_light_pdf, brdf_pdf);
        contribution_by_sample_lights =
            GfCompMult(sample_light_luminance, brdfVal) *
            abs(GfDot(si.shadingNormal, wi)) * mis_weight / sample_light_pdf;
    }

    GfVec3f contribution_by_sample_brdf{ 0 };
    GfVec3f sampled_brdf_dir;
    float sampled_brdf_pdf = 0.0f;
    auto sampled_brdf_val =
        si.Sample(sampled_brdf_dir, sampled_brdf_pdf, uniform_float);

    if (sampled_brdf_pdf > 1e-6f &&
        GfDot(si.shadingNormal, sampled_brdf_dir) > 0.0f) {
        GfRay brdf_ray;
        brdf_ray.SetPointAndDirection(
            si.position + 0.0001f * si.geometricNormal, sampled_brdf_dir);

        GfVec3f light_pos;
        Color light_radiance;
        float light_depth;
        auto* light =
            IntersectClosestLight(brdf_ray, light_pos, light_radiance, light_depth);

        SurfaceInteraction scene_si;
        bool hit_surface = Intersect(brdf_ray, scene_si);
        float surface_depth = hit_surface
                                  ? (scene_si.position - GfVec3f(brdf_ray.GetStartPoint()))
                                        .GetLength()
                                  : std::numeric_limits<float>::infinity();

        if (light != nullptr &&
            light_depth < std::numeric_limits<float>::infinity() &&
            light_depth < surface_depth) {
            float light_pdf = light->PdfLi(
                si.position + 0.0001f * si.geometricNormal, sampled_brdf_dir);
            light_pdf *= 1.0f / float(render_param->lights->size());
            float mis_weight = PowerHeuristic(sampled_brdf_pdf, light_pdf);
            contribution_by_sample_brdf =
                GfCompMult(light_radiance, sampled_brdf_val) *
                abs(GfDot(si.shadingNormal, sampled_brdf_dir)) * mis_weight /
                sampled_brdf_pdf;
        }
    }

    return contribution_by_sample_lights + contribution_by_sample_brdf;
}
