...
{
    auto wi_local = WorldToTangent(wi);
    auto wo_local = WorldToTangent(this->wo);
    return material->Eval(wi_local, wo_local, texcoord);
}

inline float SurfaceInteraction::Pdf(GfVec3f wi, GfVec3f wo) const
{
    return material->Pdf(
        WorldToTangent(wi), WorldToTangent(wo), texcoord);
}
